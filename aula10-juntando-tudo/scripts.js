...
A função deve imprimir uma mensagem dizendo "Parabéns, já pode sair de férias eternas :)" 
ou "Infelizmente para você, ainda falta um tempo :(". Caso seja informado o nome de um colaborador que não existe, deve ser apresentada a mensagem "Colaborador inválido :|".
...
function verificarAposentadoria(nome){
    ...
    } else if (idades[posicao] >= 65 || temposDeTrabalho[posicao] >= 30){
        return console.log("Parabéns, já pode sair de férias eternas :)")
    } else if (idades[posicao] >= 60 && temposDeTrabalho[posicao] >= 25) {
        return console.log("Parabéns, já pode sair de férias eternas :)") 
    } else {
...
verificarAposentadoria('Rosiméri')    
...
verificarAposentadoria('Cléverson')   
verificarAposentadoria('Nádio')      
verificarAposentadoria('Wellison')   
